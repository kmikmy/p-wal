...
pageFix(int page_id, int th_id)
{
  ...
  if(!isPageFixed(page_id)){
    ...
    */
    dirty_page_table.add(pbuf->page_id, Logger::readLSN(), Logger::currentOffsetLogFile(th_id),th_id);
    pbuf->fixed_count=1;
  }
  else{ // 既にfixされているなら
    pbuf->fixed_count++;
...
flushPage()
{
  ...
  for(int i=0;i<PAGE_N;i++){
    if(!isPageFixed(i)) // 正確にはis_page_modifiedにしなければならない。またfixされているページの情報はD.P.Tの内容を見るだけでよいはずだ。
      continue;
